...
namespace vulkan {
    ...
    VulkanBuffer::VulkanBuffer( VulkanDevice& device, VkDeviceSize instanceSize, uint32_t instanceCount, VkBufferUsageFlags usageFlags, VkMemoryPropertyFlags memoryPropertyFlags, VkDeviceSize minOffsetAlignment) : vulkanDevice{ device }, instanceSize{ instanceSize }, instanceCount{ instanceCount }, usageFlags{ usageFlags }, memoryPropertyFlags{ memoryPropertyFlags } {
        alignmentSize = (minOffsetAlignment > 0)
            ? getAlignment(instanceSize, minOffsetAlignment)
            : instanceSize;  // Fallback to raw size if no alignment needed
    ...
    void VulkanBuffer::writeToBuffer(void* data, VkDeviceSize size, VkDeviceSize offset) {
        ...
        assert(size <= bufferSize && "writeToBuffer size exceeds buffer allocation!");
