...
namespace vulkan {
	...
	PointLightSystem::PointLightSystem(VulkanDevice& device, VkRenderPass renderPass, VkDescriptorSetLayout globalSetLayout) : vulkanDevice{ device } {
	...
	void PointLightSystem::createPipeline(VkRenderPass renderPass) {
		...
		vulkanPipeline = std::make_unique<VulkanPipeline>(vulkanDevice, "shaders/point_light.vert.spv", "shaders/point_light.frag.spv", pipelineConfig);
	}
	...
	void PointLightSystem::update(FrameInfo& frameInfo, GlobalUbo& ubo) {
		...
		for (auto& keyValue : frameInfo.gameObjects) {
			...
			assert(lightIndex < MAX_LIGHTS && "Number of point lights exceeds specified maximum");
	...
	void PointLightSystem::render(FrameInfo& frameInfo) {
		// Sort lights
		std::map<float, objects::GameObject::id_t> sorted;
		for (auto& keyValue : frameInfo.gameObjects) {
			auto& obj = keyValue.second;
			if (obj->pointLight == nullptr) continue;
